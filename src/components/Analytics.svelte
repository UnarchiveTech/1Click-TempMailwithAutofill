<script lang="ts">
  ...
  let {
    onBack = () => {},
    analytics = { createdAt: null, inboxesCreated: 0, emailsReceived: 0, otpsDetected: 0, notificationsSent: 0 },
    loading = false,
    onLoadAnalytics = () => {}
  } = $props<{
    onBack?: () => void;
...
<div class="flex-1 overflow-y-auto px-4 py-4 space-y-5 pb-20">
  ...
  <section class="space-y-2">
    ...
    <div class="bg-base-100 rounded-2xl px-4 py-3 flex items-center justify-between">
      ...
      <div class="text-2xl font-bold text-primary">{analytics.inboxesCreated}</div>
    </div>
  ...
  <!-- ── Refresh Button ── -->
  <button class="w-full btn btn-outline rounded-2xl h-12 text-sm font-semibold" onclick={onLoadAnalytics}>
